...
class SentimentLDAGibbsSampler:
    ...
    def _initialize_(self, reviews, labels, unlabeled_reviews):
        ...
        self.ts_distribution = np.zeros((self.numTopics, self.numSentiments))
        self.topics = {}
        self.sentiments = {}
        self.sentimentprior = {}
        ...
        self.ts_distribution = (self.n_ts + self.gammaVec) / \
                (self.n_t + np.sum(self.gammaVec))[:,np.newaxis]
        ...
        for iteration in range(maxIters):
            ...
            for d in range(numDocs):
                ...
                if iteration == maxIters - 1:
                    self.dt_distribution[d,:] = (self.n_dt[d] + self.alphaVec) / \
                    (self.n_d[d] + np.sum(self.alphaVec))
                    self.t_distribution = (self.n_ts + self.gammaVec) / (self.n_t + np.sum(self.gammaVec))[:,np.newaxis]
                    self.dt_distribution = self.dt_distribution/np.sum(self.dt_distribution, axis=1)[:,np.newaxis]
                    self.ts_distribution = self.ts_distribution/np.sum(self.ts_distribution, axis=1)[:,np.newaxis]
            ...
            #for k in range(self.numTopics):
            #    loglikelihood += np.sum(gammaln((self.n_ts[k, :] + self.gammaVec))) - gammaln(np.sum(self.n_ts[k, :] + self.gammaVec))
...
def run_experiment(numsentilabel,numtopics,alpha,beta,gamma,maxiter,numwordspertopic):
    ...
    for i in range(len(test_review)):
        ...
        temp = np.matmul(sampler.dt_distribution[index,:],sampler.ts_distribution)
        for k, val in enumerate(temp):
            sentiment += (k+1)*binsize*val
        ds_estimated.append(sentiment/np.sum(temp))
    ...
    #topic_sentiment_df.to_csv(review_data_file.replace('.csv',"_{}_iter_output_ts.csv".format(maxiter)), index=False)
    
    #for i in range(0,5):
    #    index = len(train_review) + i
    #    post_processing(test_review.iloc[i],index,sampler,worddict)
        #save_document_image('../output/{}_review_ts_{}.png'.format(review_data_file.split('/')[-1].replace('.csv',''),index+1),sampler.ts_distribution)
