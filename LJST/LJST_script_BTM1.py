...
class SentimentLDAGibbsSampler:
    ...
    def processReviews(self, reviews, window=5):
        ...
        self.vectorizer = CountVectorizer(analyzer="word",tokenizer=None,preprocessor=None,stop_words="english",max_features=MAX_VOCAB_SIZE,max_df=.75,min_df=10,ngram_range=(1,2))
...
def run_experiment(numsentilabel,numtopics,skipgramwindow,alpha,beta,gamma,maxiter,numwordspertopic):
    global sampler, ds_estimated, rmse, coh_score, topic_sentiment_df, topic_df
    ...
    worddictpertopic = sampler.getTopKWordsperTopic(numwordspertopic)
    ...
    #topic_df = topic_sentiment_df.groupby(['topic_id'])['top_words'].apply(lambda x: ','.join(list(set(",".join(x).split(','))))).reset_index()
    topic_df = pd.DataFrame([],columns=['topic_id','top_words'])
    topic_df['topic_id'] = np.arange(1,numtopics+1)
    topic_df['top_words'] = list(worddictpertopic.values())
    #for i in range(0,5):
    #    index = len(train_review) + i
    ...
    '''
    print ("RMSE, MAPE, Coherence ,Hscore values are {},{}%,{}".format(rmse,mape,coh_score))
...
if __name__ == '__main__':
    ...
    testsize = .2
    review_data_file = 'C:/Users/asengup6/Documents/Work/Research/LJST/data/train_sentiment.csv'
    ...
    '''
    test_data['topic_id1'] = [sampler.n_dt[i].argmax() for i in range(len(train_data),len(train_data)+test_size)]
    test_data['topic_id2'] = [sampler.n_dt[i].argsort()[-2] for i in range(len(train_data),len(train_data)+test_size)]
    test_data['topic_id3'] = [sampler.n_dt[i].argsort()[-3] for i in range(len(train_data),len(train_data)+test_size)]
    
    test_data['topic_probability1'] = [sampler.dt_distribution[i,test_data['topic_id1'].iloc[i-len(train_data)]] for i in range(len(train_data),len(train_data)+test_size)]
    test_data['topic_probability2'] = [sampler.dt_distribution[i,test_data['topic_id2'].iloc[i-len(train_data)]] for i in range(len(train_data),len(train_data)+test_size)]
    test_data['topic_probability3'] = [sampler.dt_distribution[i,test_data['topic_id3'].iloc[i-len(train_data)]] for i in range(len(train_data),len(train_data)+test_size)]
    
    binsize = sentirange*1.0/numsentilabel
    def calculate_sentiment(docuid,topicid):
        sentiment = 0
        temp = sampler.dts_distribution[docuid,topicid]
        for k, val in enumerate(temp):
            sentiment += k*binsize*val
            
        return np.round(sentiment*(test_data.sentiment_score.max() - test_data.sentiment_score.min())/sentirange)
    
    test_data['sentiment_score1'] = [calculate_sentiment(i,test_data.topic_id1.iloc[i-len(train_data)]) for i in range(len(train_data),len(train_data)+test_size)]
    test_data['sentiment_score2'] = [calculate_sentiment(i,test_data.topic_id2.iloc[i-len(train_data)]) for i in range(len(train_data),len(train_data)+test_size)]
    test_data['sentiment_score3'] = [calculate_sentiment(i,test_data.topic_id3.iloc[i-len(train_data)]) for i in range(len(train_data),len(train_data)+test_size)]
    
    topic_df.to_csv('{}_LJST_topic_df.csv'.format(review_data_file.replace('.csv','')),index=False)
    
    test_data.to_csv('{}_LJST_output.csv'.format(review_data_file.replace('.csv','')),index=False)
    '''
    '''
